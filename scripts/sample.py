...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt_path', type=str, default='DecompDiff/checkpoints/290000.pt')
    ...
    parser.add_argument('--prior_mode_dict', type=str, default='utils/prior_map.pkl')
    args = parser.parse_args()
    RDLogger.DisableLog('rdApp.*')

    # args.db_path = './data/crossdocked_v1.1_rmsd1.0_processed'
    # args.ori_data_path = './data/crossdocked_v1.1_rmsd1.0'
    args.split_path = './data/split_by_name.pt'
    # args.index_path = './data/test_index.pkl'

    # Load Prior
    if args.prior_mode is None:
        prior_mode_dict = pickle.load(open(args.prior_mode_dict, 'rb'))
        args.prior_mode = prior_mode_dict[args.data_id]
    ...
    log_dir = os.path.join(args.outdir, '%s-%s-%03d' % (config_name, prior_mode, args.data_id))

    # filter generated data
    if os.path.exists(os.path.join(log_dir, 'result.pt')):
    ...
    # ckpt['config'].data.path = args.ori_data_path
    ckpt['config'].data.split = args.split_path
    ckpt['config'].data.version = 'ref_prior_aromatic'
