...
def sample_diffusion_ligand_decomp(
    ...
    results = []
    for i, (pred_pos, pred_v, pred_bond_index, pred_bond_type) in enumerate(
        zip(all_pred_pos, all_pred_v, all_pred_bond_index, all_pred_bond)):
...
if __name__ == '__main__':
    ...
    filename = config_name + '_' + str(args.data_id).zfill(3)
    ...
    ckpt['config'].data.path = args.db_path
    ...
    for r in raw_results:
        results.append({
            **r,
            'ligand_filename': data.src_ligand_filename
        })
