...
def _which_or_raise(candidates):
    for c in candidates:
        p = shutil.which(c)
        if p:
            return p
    ...
    if conda_prefix:
        for c in candidates:
            p = os.path.join(conda_prefix, "bin", c)
            if os.path.isfile(p) and os.access(p, os.X_OK):
                return p
    raise FileNotFoundError(
        f"Could not find any of {candidates} in PATH"
        + (f" or {os.path.join(conda_prefix, 'bin')}." if conda_prefix else ".")
    )
...
class PrepProt(object): 
    ...
    def addH(self, prot_pqr):  # call pdb2pqr
        ...
        pdb2pqr_bin = _which_or_raise(["pdb2pqr30", "pdb2pqr"])
        subprocess.Popen([pdb2pqr_bin, '--ff=AMBER', self.prot, self.prot_pqr],
