...
from models.common import GaussianSmearing, MLP, batch_hybrid_edge_connection, outer_product, AngularEncoding
...
class AttentionLayerO2TwoUpdateNodeGeneral(nn.Module):
    ...
    def forward(self, h, x, edge_attr, edge_index, protein_bond_index, h_protein_bond, mask_ligand, e_w=None):
        ...
        if h_protein_bond is not None:
            # -> protein graph message passing
            new_h_with_bond = self.node_layer_with_bond(h, h_protein_bond, protein_bond_index)  # ht+1 '' = f(ht, bt, xt)
            new_h_bond = h_protein_bond + self.bond_layer(h, h_protein_bond, x, protein_bond_index)  # bt+1 = h(ht, bt, xt)
            # --> update h node
            new_h = h + self.lin_node(new_h_with_edge + new_h_with_bond)
        else:
            new_h_bond = None
            new_h = h + self.lin_node(new_h_with_edge)

        # update x
        delta_x_with_edge = self.pos_layer_with_edge(new_h, rel_x, edge_feat, edge_index, e_w=e_w)   # g(xt, ht+1, ..)
        if h_protein_bond is not None:
            bond_src, bond_dst = protein_bond_index
            rel_bond_x = x[bond_dst] - x[bond_src]
            delta_x_with_bond = self.pos_layer_with_bond(new_h, rel_bond_x, new_h_bond, protein_bond_index)
            delta_x = delta_x_with_edge + delta_x_with_bond
        else:
            delta_x = delta_x_with_edge
        x = x + delta_x * ~mask_ligand[:, None]  # only target positions will be updated

        return new_h, new_h_bond, x


class UniTransformerO2Protein(nn.Module):
    ...
    def _build_edge_type(self, edge_index, mask_type):
        """
        Args:
            edge_index: (2, E)
            mask_ligand: (N, )
        """
        # denote ll, lp, pl, pp edge type
        src, dst = edge_index
        edge_type = torch.zeros(len(src)).to(edge_index)
        n_src = mask_type[src] == 1
        n_dst = mask_type[dst] == 1
    ...
    def forward(self, h, x, protein_bond_index, h_protein_bond,
                mask_ligand, batch, return_all=False):
        all_x = [x]
        all_h = [h]
        all_h_bond = [h_protein_bond]

        for b_idx in range(self.num_blocks):
            # build knn graph for pl
            edge_index = self._connect_edge(x, ~mask_ligand, batch)
            edge_type = self._build_edge_type(edge_index, ~mask_ligand) 
            # get distance feature for knn graph
            if self.use_global_ew:
                src, dst = edge_index
            ...
            for l_idx, layer in enumerate(self.base_block):
                h, h_protein_bond, x = layer(h, x, edge_type, edge_index, protein_bond_index, h_protein_bond, mask_ligand, e_w=e_w)
            
            all_x.append(x)
            all_h.append(h)
            all_h_bond.append(h_protein_bond)

        # edge_index = self._connect_edge(x, mask_ligand, batch)
        outputs = {'x': x, 'h': h, 'h_bond': h_protein_bond}
        if return_all:
