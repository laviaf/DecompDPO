...
from models.transitions import cosine_beta_schedule, get_beta_schedule, DiscreteTransition, index_to_log_onehot, \
    log_sample_categorical, segment_schedule, advance_schedule
...
class DecompScorePosNet3D(nn.Module):
    ...
    def __init__(self, config, protein_atom_feature_dim, ligand_atom_feature_dim, num_classes,
        ...
        if 'diff_pos' in config:
            if config.diff_pos.beta_schedule == 'cosine':
                alphas = cosine_beta_schedule(config.num_diffusion_timesteps, config.pos_beta_s) ** 2
                print('cosine pos alpha schedule applied!')
                betas = 1. - alphas
            elif config.diff_pos.beta_schedule == 'advance':
                scale_start = dict.get(config.diff_pos, 'scale_start', 0.999)
                scale_end = dict.get(config.diff_pos, 'scale_end', 0.001)
                width = dict.get(config.diff_pos, 'width', 2)
                betas = advance_schedule(config.num_diffusion_timesteps, scale_start, scale_end, width)
                print('advance pos alpha schedule applied!')
                alphas = 1. - betas
            else:
                betas = get_beta_schedule(
                    beta_schedule=config.diff_pos.beta_schedule,
                    beta_start=config.diff_pos.beta_start,
                    beta_end=config.diff_pos.beta_end,
        ...
        if 'diff_atom' in config:
            if config.diff_atom.beta_schedule == 'cosine':
                self.atom_type_trans = DiscreteTransition(
                    noise_schedule=config.diff_atom.beta_schedule, num_timesteps=self.num_timesteps,
                    s=config.diff_atom.beta_s, num_classes=self.num_classes, prior_probs=prior_atom_types
                )
            elif config.diff_atom.beta_schedule == 'advance':
                print('Use advance beta schedules')
                scale_start = dict.get(config.diff_atom, 'scale_start', 0.999)
                scale_end = dict.get(config.diff_atom, 'scale_end', 0.001)
                width = dict.get(config.diff_atom, 'width', 2)
                node_betas = advance_schedule(config.num_diffusion_timesteps, scale_start, scale_end, width)
                self.atom_type_trans = DiscreteTransition(
                    betas=node_betas, num_timesteps=self.num_timesteps,
                    num_classes=self.num_classes, prior_probs=prior_atom_types
                )
            else:
                raise NotImplementedError
        else:
            self.atom_type_trans = DiscreteTransition(
                noise_schedule=config.v_beta_schedule, num_timesteps=self.num_timesteps,
                s=config.v_beta_s, num_classes=self.num_classes, prior_probs=prior_atom_types
            )
        
        # Moldiff bond schedule
        if 'diff_bond' in config:
            if config.diff_bond.beta_schedule == 'cosine':
                self.bond_type_trans = DiscreteTransition(
                    noise_schedule=config.diff_bond.beta_schedule, num_timesteps=self.num_timesteps,
                    s=config.diff_bond.beta_s, num_classes=self.num_bond_classes, prior_probs=prior_bond_types
                )
            elif config.diff_bond.beta_schedule == 'advance':
                print('Use advance beta schedules')
                scale_start = dict.get(config.diff_bond, 'scale_start', 0.999)
                scale_end = dict.get(config.diff_bond, 'scale_end', 0.001)
                width = dict.get(config.diff_bond, 'width', 2)
                bond_betas = advance_schedule(config.num_diffusion_timesteps, scale_start, scale_end, width)
                self.bond_type_trans = DiscreteTransition(
                    betas=bond_betas, num_timesteps=self.num_timesteps,
                    num_classes=self.num_bond_classes, prior_probs=prior_bond_types
                )
            elif config.diff_bond.beta_schedule == 'segment':
                print('Use segment beta schedules')
                bond_betas = segment_schedule(config.num_diffusion_timesteps, config.diff_bond['time_segment'], config.diff_bond['segment_diff'])
                self.bond_type_trans = DiscreteTransition(
                    betas=bond_betas, num_timesteps=self.num_timesteps,
                    num_classes=self.num_bond_classes, prior_probs=prior_bond_types
                )
    ...
    def compute_v_Lt_atom_wise(self, log_v_model_prob, log_v0, log_v_true_prob, t, batch):
        kl_v = categorical_kl(log_v_true_prob, log_v_model_prob)  # [num_atoms, ]
        decoder_nll_v = -log_categorical(log_v0, log_v_model_prob)  # L0
        assert kl_v.shape == decoder_nll_v.shape
        mask = (t == 0).float()[batch]
    ...
    def get_diffusion_loss(
            ...
            ligand_decomp_batch, prior_centers, prior_stds, 
            prior_num_atoms=None, batch_prior=None,
        ...
        if dpo:
            kl_v = self.compute_v_Lt_atom_wise(log_v_model_prob=log_v_model_prob, log_v0=log_ligand_v0,
        ...
        if self.bond_diffusion:
            ...
            if dpo:
                kl_b = self.compute_v_Lt_atom_wise(log_v_model_prob=log_b_model_prob, log_v0=log_ligand_b0,
        ...
        if self.loss_pos_type == 'mse':
            ...
            if dpo:
                loss_pos = (((pred - target) ** 2) / (batch_noise_stds ** 2)).sum(-1)
            else:
    ...
    def get_diffusion_dpo_loss(
            self, protein_pos, protein_v, batch_protein,
            ligand_pos, ligand_v_aux, batch_ligand, prior_centers,
            ligand_pos_perturbed, ligand_v_perturbed, log_ligand_vt, ligand_b_perturbed, log_ligand_bt,
            log_ligand_v0, log_ligand_b0, pos_noise, batch_noise_stds,
            batch_prior=None, ligand_fc_bond_index=None, batch_ligand_bond=None, ligand_atom_mask=None,
            time_step=None,
            dpo=False, return_prediction=False
    ):
        protein_pos, ligand_pos_perturbed, offset = center_pos(
            protein_pos, ligand_pos_perturbed, batch_protein, batch_ligand, mode=self.center_pos_mode)
        ligand_pos = ligand_pos - offset[batch_ligand]
    ...
    def adjust_pos_with_energy_drift(self, ligand_pos, energy_drift_opt, prior_centers, ligand_v_next,
        ligand_decomp_batch, batch_ligand, ligand_decomp_index,
        offset, full_protein_pos, full_batch_protein, pos_model_mean,
        i, t):
        all_energy_grad = 0.
        xt = ligand_pos
    ...
    def sample_diffusion(self, protein_pos, protein_v, batch_protein,
                         init_ligand_pos, init_ligand_v, ligand_v_aux, batch_ligand,
                         prior_centers, prior_stds,
        ...
        for i in tqdm(time_seq, desc='sampling', total=len(time_seq)):
            ...
            if energy_drift_opt is not None:
                pos_model_mean = self.adjust_pos_with_energy_drift(ligand_pos, energy_drift_opt, prior_centers, ligand_v_next, 
                ligand_decomp_batch, batch_ligand, ligand_decomp_index,
                offset, full_protein_pos, full_batch_protein, pos_model_mean,
                i, t)
    ...
    @torch.no_grad()
    def diffuse_recon(self, protein_pos, protein_v, batch_protein, protein_group_idx,
        ...
        assert diffuse_steps <= self.num_timesteps
        time_seq = list(reversed(range(0, diffuse_steps)))
        for i in tqdm(time_seq, desc='sampling', total=len(time_seq)):
            t = torch.full(size=(num_graphs,), fill_value=i, dtype=torch.long, device=protein_pos.device)
            preds = self(
                protein_pos=protein_pos,
